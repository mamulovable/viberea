...
export default function DashboardPage() {
  ...
  /**
   * Map of projectId → files for live preview thumbnails.
   * undefined means "still loading", a Record means "loaded".
   */
  ...
  /**
   * After projects are loaded, fetch files for each project in parallel.
   * Files are used for live Sandpack preview thumbnails on the cards.
   * Failures are silently caught — the card falls back to a gradient.
   * Uses a mounted flag to prevent state updates after navigation.
   */
  useEffect(() => {
  ...
  async function handleCreateProject(data: CreateProjectData) {
    try {
      ...
      try {
        ...
        // sessionStorage unavailable — editor will just open without auto-send
      }
